...
#define _SVID_SOURCE
#define D_GNU_SOURCE
#define PERM (S_IRUSR | S_IWUSR) /* this is for flag of SHMGET which is taken from UNIX book */
...
*****
*/
/* all shared memory sections are taken from example code in UNIX System Programming -Chapter 15.3.4
Monitoring of a shared memory Section. 
due respective owners 
...
This program also creates shared memory for all purposes PIPE  did before. Such as IPC communication 
...
subDIR
 -file.3
 -file.4
>>in this example all files communicate with their respective parent directory via IPC Shared Memory 
so in the end you will get number of all created directories as number of shared memory queues.

all subDIR's collect their program outputs then deliver to behold directory.

in program output example you get 10 prints to terminal screen with their respective values.


UGURKAN ATES 150144012 Deadline 12 May 2017 */
...
int show_dir_content(char * path,int argc,char **argv)
{
	...
	if ((shared_totale = (int *)shmat(id, NULL, 0)) == (void *)-1) {
    ...
    while ((kek = readdir(dsayac)) != NULL) {
    	 srand (time(NULL));
         bytes_read=rand();
    	while(bytes_read<100000  && bytes_read>524288)
       	 bytes_read=rand();
...
void print_to_screen(struct total a){
    ...
    printf("Created SHARED Memory Queue number and size %d %ld\n",a.number_of_memory,shared_total/  10);
...
void struct_to_thread(struct total *a,long timedif){
    ...
    a->number_of_memory=a->number_of_director;
    /*size of memory=?*/
    if(signalcheck==0)
